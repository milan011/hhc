<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column :label="$t('permission.name')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('permission.guard_name')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.guard_name }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('permission.description')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.description }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('permission.group')" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px;">
        ...
        <el-form-item :label="$t('permission.guard_name')" prop="guard_name">
  ...
  methods: {
    ...
    handleModifyStatus(row, status) {
      this.$confirm('确定要删除?', '提示', {
        ...
        deletePermission(this.temp).then((response) => {
